...
class ServiceComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    if (this.rolId === 1) {
      this.itemsService.getItems(this.sharedService.getModeConsultantItems().value,false)
      .then(this.handleShowServices.bind(this))
      .catch(this.catchShowServicesError);
  ...
  handleShowServices(response: any) {
    let items = response;
    items.map((item) => {
  ...
  catchShowServicesError(err: any) {
    console.log(err);
  }
  ...
  showService(item: any) {
    if (this.rolId === 1) {
      ...
      this.router.navigate(['/show-service', item, this.organizationSelected, this.userId]);
    }
  }

  createProduct() {
    this.router.navigate(['/create-service', this.organizationSelected, this.userId]);
  }
  ...
  editService(itemId: number) {
    this.router.navigate(['/update-service', itemId, this.organization.idPk, this.userId]);
  }
